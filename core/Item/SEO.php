...
//        $keywords[] = ($v = $this->getItem()->getColor()) ? $v->getName() : '';
    ...
    public function managePage(Layout $view, array $params = [])
    {
        ...
        if ($this->app->managers->items->getBrand($item)->isNoIndex()) {
...
//            ->addMetaProperty('og:locale', strtolower($this->app->translator->getLocale()))
            ...
            ->addMetaProperty('og:image', $imageLink = $image->getLink())
...
//            ->addMetaProperty('product:size', $this->app->managers->items->getSizes($item))
    ...
    {
        $view->addBreadcrumb($this->app->translator->makeText('catalog.catalog'), (string)new CatalogURI);
